  ...
  fetch(`/movies/${imdbID}`, { method: 'PUT' })
    .then(response => {
      if (response.status === 201) {
        const resultEntry = document.querySelector(
            `#searchResults [data-imdbid="${imdbID}"]`
        );
  ...
  function renderUserGreeting() {
    ...
    if (currentSession) {
      ...
      greetingElement.textContent =
          ...
          `du hast dich am ${loginDate.toLocaleDateString('de-DE')} ` +
          `um ${loginDate.toLocaleTimeString('de-DE', {
            hour: '2-digit',
            minute: '2-digit'
          })} angemeldet.`;
    } else {
